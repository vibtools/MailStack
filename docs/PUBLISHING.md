...
4. Build and verify the deterministic release archive:
   ...
   ```bash
   python scripts/build_release.py --root . --version 1.3.4-rc.3
   python scripts/verify_release.py \
     dist/mailstack-1.3.4-rc.3-source.zip \
     --checksum dist/mailstack-1.3.4-rc.3-source.zip.sha256
   ```
...
git init
git add .
git commit -m "Prepare MailStack 1.3.4-rc.3 candidate"
git branch -M main
...
```bash
git tag -a v1.3.4-rc.3 -m "MailStack 1.3.4-rc.3"
git push origin v1.3.4-rc.3
```
...
- `mailstack-1.3.4-rc.3-source.zip`
- `mailstack-1.3.4-rc.3-source.zip.sha256`
...
`v1.3.4-rc.1` remains immutable historical PHASE-006 pre-release evidence; `1.3.4-rc.3` is the current
