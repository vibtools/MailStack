...
2. Run `python scripts/forensic_audit.py --root . --profile essential --full` in the supported Python 3.12 environment. Run the repository hygiene profile manually when documentation, design, or inventory files change.
...
4. Build and verify the deterministic release archive:
    ...
    ```bash
    python scripts/build_release.py --root . --version 1.3.5.5
    python scripts/verify_release.py \
    dist/mailstack-1.3.5.5-source.zip \
    --checksum dist/mailstack-1.3.5.5-source.zip.sha256
    ```
...
git init
git add .
git commit -m "Prepare MailStack 1.3.5.5 release"
git branch -M main
git remote add origin <YOUR-REPOSITORY-SSH-OR-HTTPS-URL>
git push -u origin main
```

Create the annotated pre-release tag only after the intended release commit is the current
`main` head and the exact `main` SHA has a successful push CI run:

```bash
git tag -a v1.3.5.5 -m "MailStack 1.3.5.5"
git push origin v1.3.5.5
```
...
- `mailstack-1.3.5.5-source.zip`
- `mailstack-1.3.5.5-source.zip.sha256`
...
`v1.3.5-rc.1` remains immutable historical PHASE-006 pre-release evidence; `1.3.5.5` is the current
