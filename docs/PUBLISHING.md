...
4. Build and verify the deterministic release archive:
    ...
    ```bash
    python scripts/build_release.py --root . --version 1.3.5.2
    python scripts/verify_release.py \
     dist/mailstack-1.3.5.2-source.zip \
     --checksum dist/mailstack-1.3.5.2-source.zip.sha256
    ```
...
git init
git add .
git commit -m "Prepare MailStack 1.3.5.2 candidate"
git branch -M main
...
```bash
git tag -a v1.3.5.2 -m "MailStack 1.3.5.2"
git push origin v1.3.5.2
```
...
- `mailstack-1.3.5.2-source.zip`
- `mailstack-1.3.5.2-source.zip.sha256`
...
`v1.3.5-rc.1` remains immutable historical PHASE-006 pre-release evidence; `1.3.5.2` is the current
