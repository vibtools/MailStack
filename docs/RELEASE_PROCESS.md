...
8. Build with `python scripts/build_release.py --root . --version 1.3.0-rc.5` and verify the ZIP/checksum with `python scripts/verify_release.py`.
...
`workflow_dispatch` is intentionally build/validation-only. Release automation never edits or clobbers an existing release. A release is not promoted from release candidate to production-ready until the clean-VPS acceptance test passes.
