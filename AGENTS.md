...
3. **MANDATORY CI WORKFLOW & FORENSIC INVENTORY VALIDATION:**
   ...
   - **Release Version Is Not an IP Literal:** Never scan, verify, or reject the canonical `VERSION`, package version, release tag version, or archive top-level version directory as a global IP literal. Version strings such as `1.3.5.1` are semantic release identifiers, not network addresses. IP scanning must continue to reject actual global IP addresses everywhere else, and this exemption must remain covered by regression tests.
